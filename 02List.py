...
리스트(list)
    : 대괄호[] 안에 콤마로 항목을 구분하여, 대입연사자로 항목을 변경할 수 
    있는 시퀀스(순서를 가지고 있는) 자료형이다. 서로 다른 자료형의 항목으로도 
    구성할 수 있다. 인덱싱, 슬라이싱, 연결, 반복 등이 가능하다.
    Mutable데이터 타입이라고 한다.
'''
...
list1.remove(99) #마지막 항목 반환 및 삭제
print('remove=>', list1)

list1.reverse() #리스트를 뒤집을 때 사용
print('reverse=>', list1)

'''
List Comprehension
    : 대괄호 안에 for루프로 반복적인 표현식을 실행해서 리스트 요소들을
    초기화 하는 방법
    형식]
