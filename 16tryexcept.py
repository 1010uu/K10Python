...
def calc(val):
    ...
    try: #예외발생이 예상되는 지점을 try로 묶는다.
        ...
        if val[0]==100:
            print(no_var) #선언되지 않는 변수를 출력하므로 예외가 발생
        elif val[0]==55:
            result=val[0]/0 #숫자를 0으로 나누므로 예외가 발생
            print("결과", result)
    #예외명을 명시하여 특정 예외만 처리한다.
    except IndexError:
        print('리스트의 인덱스에 에러가 있습니다.')
    #에외발생시의 에러메세지를 변수로 받아서 출력한다. 
    except NameError as err:
        print('선언되지 않은 변수를 사용하였습니다.', str(err))
    #예외명을 명시하지 않으면 모든 예외를 처리할 수 있는 블럭이 된다.
    except:
        print('예외가 발생했습니다.')
    #예외가 발생되지 않았을 때 실행되는 블럭
    else:
        print('에러 없음')
    #try로 진입했을 때 예외 발생과 상관 없이 무조건 실행되는 블럭
    finally:  
        print('sum',sum)
...
try:
    ...
    try:
        ...
        print("파일 객체를 닫습니다.")
        fp.close()
except IOError: #현재 경로에 파일이 없으므로 예외가 발생
    print('파일 에러 발생')
