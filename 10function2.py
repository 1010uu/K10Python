...
print("sum(10,20)호출 후 반환 값=", sum(10,20)) #합의 결과 30 출력
...
#2. 키워드 매개변수 : 순서와 상관 없이 매개변수명에 따라 전달된다.
...
def defaultArgs(lan="Java"):
    ...
    return
defaultArgs() #이 경우 Java 출력
defaultArgs("C++") #여기선 C++ 출력
#만약 Java에서의 호출이라면 첫번째 호출에서 에러가 발생하게 될 것

'''
4. 가변 매개변수
    : 여러 개의 매개변수를 전달해야 할 때 사용하는 매개변수로
    Java의 가변인자와 비슷하다.
    *를 이용해서 매개변수 값을 튜플로 그룹화한다.
    **를 이용해서 매개변수를 딕셔너리로 사용할 수 있다.
'''
def product(*args):
    print("*args=>", args) #튜플형태로 출력한다.
    result=1
    for a in args: #튜플이므로 반복 가능
        result *=a #각 원소를 통해 누적 곱을 계산
    return result
print("product(1,2,3,4)=", product(1,2,3,4))

def famousMan(**man):
    print('**man', man)  #딕셔너리 형태로 출력
