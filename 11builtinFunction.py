...
Built-in Function(내장함수)
    : 내장함수는 외부모듈과 달라 import가 필요하지 않기 때문에
...
이터레이터(iterator)
    ...
    iter() : 반복을 끝낼 값(sentinel)을 지정하면 특정값이 나올 때 
        반복을 종료하는 함수
        형식]
...
while True: #무한루프
    try:
        print('it=', next(it)) #반복할 항목이 있을 때 하나씩 반환
    except StopIteration: #반복할 항목이 없다면 예외 발생
        print("예외발생")
...
def number_generator1(): #제너레이터 함수 정의
    ...
    yield 1 #next()로 지속적으로 호출하면 순차적으로 yield가 실행돼 값이 반환된다.
    yield 2
...
def number_generator2(n):
    i=0
    while i<n:
        yield i  #yield가 실행될 때 현재 i값이 for문으로 전달되어 출력된다.
        i+=1
for i in number_generator2(5): #for문에서 호출할 때 자동으로 next()가 실행된다.
    print(i)
#실행결과 : 0 1 2 3 4
