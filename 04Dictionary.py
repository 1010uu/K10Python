...
딕셔너리(Dictionary)
    ...
    고유키(key)와 값(values)으로 지정된다. 값은 변경할 수 있고, 중괄호{}
    로 선언한다.
'''
#생성1 : dict()함수를 이용해 딕셔너리 생성
...
for key in fruits: #key를 먼저 얻어온 후
    val = fruits[key] #얻어온 key를 통해 value를 출력
    print("%s:%d" % (key,val))
...
#keys() : 딕셔너리의 키로 된 dict_keys객체를 반환한다.
get_keys = fruits.keys()
for k in get_keys:
    print(k) #key를 출력

#values() : 딕셔너리의 값들로 된 dict_values객체를 반환한다.
