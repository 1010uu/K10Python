...
람다(lamda)함수
    ...
    부른다. 이름이 없으므로 익명함수로 부르기도 한다. lamda키워드를
...
람다식 자체 호출
    ...
    바로 호출할 수 있다.
'''
print("람다식 자체 호출=", (lambda x, y : x+y)(100,200))

#람다식과 map, filter, reduce함수 활용
'''
map()함수
...
'''
print("###람다식과 map함수1 ###")
multiLamda = lambda x : x*2 #인자에 2를 곱한 결과를 반환하는 람다식 정의
list_data = [1,2,3,4,-1,-2,-5,-10] #리스트 정의
#list_data의 원소 갯수만큼 람다식을 반복 호출하여 얻어진 결과를 리스트로 반환한다.
result_list=list(map(multiLamda, list_data))
...
형식]
    ...
    -조건식이 참일 때 실행문1, 거짓일때 실행문2를 사용한다.
    -람다식에서 조건부 표현식을 사용하는 :을 붙이지 않는다.
...
reduce함수
    ...
    누적해서 반환하는 함수.
    파이썬3부터는 내장함수가 아니므로 functools모듈을 import한 후 
    사용해야 한다.
'''
print("###람다식과 reduce함수 ###")
...
#operator 모듈의 add()함수를 사용하여 위와 동일한 결과 값을 반환한다.
