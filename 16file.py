'''
open()
    :파일을 다룰 때 사용되는 내장함수로 첫번째 인자 file경로만 필수인자이다.
    형식]
    ...
    mode: 파일 열기 모드
        ...
        b - 2진 모드. 바이너라모드라고도하는데 컴퓨터가 알아볼 수 있는 파일을 생성한다.
        t - 텍스트모드로 기본값이다. 사람이 알아볼 수 있는 형태의 텍스트 파일을 생성
            한다. 차후 메모장으로 열어볼 수 있다.
'''
print("="*30)
print("새파일01.txt")
print("="*30)
...
#리스트를 통해 여러줄의 내용을 입력할 때 writelines
...
with open('kosmo.p','wb')as file:
    pickle.dump(name, file) #dump()를 통해 저장한다.
