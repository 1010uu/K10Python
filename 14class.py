...
정적메서드
    ...
    @staticmethod라는 데이코레이터를 사용한다.
클래스메서드
    :정적메서드와 비슷한데, 객체 인스턴스를 의미하는 
    self대신 cls라는 클래스를 의미하는 파라미터를 전달받는다.
    cls를 통해 클래스 변수에 엑세스할 수 있다.
    @classmethod라는 데코레이터를 사용한다.
'''
class CalculatorInit:
    
    #클래스변수(클래스 전체에서 접근 가능한 지역변수)
    count=0 
    ...
    #클레스메서드 정의. cls를 통해 클래스 변수에 접근할 수 있다.
    @classmethod
    def showInfo(cls):
        print('class메서드',cls.count)
        
#fCal=CalculatorInit() ->인자가 없는 기본 생성자는 정의되지 않음.
#2개의 인자를 전달해서 객체를 생성
...
#상속:파이썬에는 별도의 키워드 없이 클래스 정의시 매개변수 형태로 상속한다.
...
class Person:
    ...
    def __init__(self,n,a,pw):
        self.name=n
        self.age=a
        #인스턴스 변수에 언더바 2개를 붙이면 private로 정의된다.
        self.__passwd=pw #생성자에서 초기화함
...
print("이름", p1.name)
print("나이", p1.age)
#private변수이므로 클래스 외부에서는 접근할 수 없어 에러가 발생.
#print("비밀번호1", p1.__passwd)
#언더바가 없는 형태의 변수가 없으므로 에러 발생.
#print("비밀번호2", p1.passwd)

#private변수는 메서드를 통해 간접적으로 접근할 수 있다.
